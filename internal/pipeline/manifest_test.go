...
// Pins the gitdr.manifest/v4 field set. A change here means the public output contract
...
func TestManifestV4Shape(t *testing.T) {
	if pipeline.ManifestSchema != "gitdr.manifest/v4" {
	...
	checkKeys(t, "destination", decode(t, top["destination"]),
		"type", "bucket", "wormMode", "wormImmutable", "wormVerdict", "wormDetails")
